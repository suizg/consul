...
type ACLIdentityProvider struct {
	...
	KubernetesCAFile            string   `json:",omitempty"`
...
type ACLServiceIdentityListEntry struct {
	Name        string
	Description string
	Type        string
...
func (a *ACL) RoleBindingRuleUpdate(rule *ACLRoleBindingRule, q *WriteOptions) (*ACLRoleBindingRule, *WriteMeta, error) {
	if rule.ID == "" {
		return nil, nil, fmt.Errorf("Must specify an ID in Role Binding Rule Creation")
	}
...
// RoleBindingRuleRead retrieves the role binding rule details.
